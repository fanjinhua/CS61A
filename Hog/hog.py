...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while score_list[who] < goal and score_list[other(who)] < goal:
        ...
        score_list[who] += take_turn(num_rolls, score_list[other(who)], dice)
        print(num_rolls)
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    for i in range(10):
        score.append(make_averaged(roll_dice)(i + 1, dice))
        if score[i] > max_score:
            max_score = score[i]
            max_index = i + 1
        print("%d dice scores %.1f on average" % (i + 1, max_score))
    return max_index
...
def swap_strategy(score, opponent_score):
    ...
    """
    "*** YOUR CODE HERE ***"
    score_now = score + take_turn(0, opponent_score)
    if score_now * 2 == opponent_score:
        return 0
    elif score_now == opponent_score * 2:
        return BASELINE_NUM_ROLLS
    else:
